...
__package__ = "GlobalIdMaker"
__description__ = "Flask app to create 1-way hashes for study anonymization"
__url__ = "https://github.com/derekmerck/GlobalIdMaker"
...
# TODO: Add sqlite database in here to keep track of assigned ids for a namespace (salting)

app = Flask(__name__)


def check_vars(reqs, args):
    # TODO: Should also check correct format for dob (i.e., xx-yy-zzzz), mrn, etc.
    values = []
    for key in reqs:
        if key not in args:
            # Failed completeness
...
@app.route('/aoi')
def get_arbitrary_object_id():
    return hash_it(request.args.values())


@app.route('/iri')
def get_institutional_record_id():
